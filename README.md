...
* Vá até a pasta `api/` e crie um arquivo `.env` baseado no `.env.example` (se houver) com as configurações do seu Banco de Dados.
* Vá até a pasta `apps/web/` e crie um arquivo `.env.local` configurando a URL da API (ex: `NEXT_PUBLIC_API_URL=http://localhost:3333`).
* Vá até a pasta `packages/database/` e crie um arquivo `.env` com a sua URL de conexão do Prisma (ex: `DATABASE_URL="mysql://root:senha@localhost:3306/lepera_db"`).
